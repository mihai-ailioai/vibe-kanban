...
This page applies to **Git-capable workspaces**: `git_worktree` and `in_place_git`.

`in_place_directory` workspaces do not show Git status or Git actions such as pull requests, push, merge, or rebase.

Git-capable workspaces provide integrated Git operations for managing your code changes.
...
When a workspace supports Git, the Git section in the right sidebar shows:
...
Create PRs directly from a Git-capable workspace.

### Creating a PR

**From the Git panel (when available):**
...
The workspace updates to show the new ahead/behind status when Git tracking is available.
