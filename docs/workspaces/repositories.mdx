...
Repositories are the codebases you work with in Git-capable workspaces. Vibe Kanban uses repository entries in `git_worktree` and `in_place_git` modes, and each selected repository keeps its own independent Git state.

<Note>
If you create an `in_place_directory` workspace, you pick a folder source instead of adding repository entries. That mode is for plain directories and does not use Git repository configuration.
</Note>

## How Repositories Work in Workspaces

When you add a repository in `git_worktree` mode, Vibe Kanban creates a **git worktree** - a separate working directory linked to your repository.
...
Each repository in a Git-capable workspace has its own:
...
This means you can have multiple Git-capable workspaces modifying the same repository on different branches without conflicts.
</Accordion>
</AccordionGroup>

## Adding Repositories

When creating or editing a `git_worktree` or `in_place_git` workspace, you can add repositories from several sources:
...
Each repository in a Git-capable workspace has a **target branch** - the branch your changes will eventually be merged into (typically `main`, `master`, or `develop`).
...
3. Select the branch - the workspace sets or creates a working branch based on this target
...
In `git_worktree` mode, a **working branch** is automatically created for each repository. In `in_place_git` mode, you work in the selected checkout's Git repository with the branch flow available for that workspace.
...
Access repository-specific actions through the command bar in Git-capable workspaces:
