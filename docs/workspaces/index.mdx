...
A **workspace** is a dedicated environment for completing a coding task. Depending on how you create it, a workspace can use a managed git worktree, an existing git checkout, or a plain directory. `git_worktree` gives you the strongest isolation because it keeps your original checkout untouched. Think of it as a dedicated "project room" where you:
...
When you create a workspace, Vibe Kanban uses the workspace mode you selected:

- **`git_worktree` (recommended)** creates a managed git worktree on a new working branch. This is the best default for most software tasks because it keeps your original checkout untouched.
- **`in_place_git`** uses your existing git checkout directly. Choose this only when you intentionally want to work inside the repo folder that already exists on disk.
- **`in_place_directory`** uses a plain folder without Git integration. Choose this for Unity projects or other directories that are not git repositories.

Git actions only appear when the workspace mode supports them. In non-Git-capable modes, actions such as branch management, pull requests, and other unsupported Git flows are hidden or unavailable.

Nothing is pushed to a remote unless you choose to do that intentionally.
</Accordion>
