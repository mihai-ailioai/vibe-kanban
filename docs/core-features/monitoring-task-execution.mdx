...
When you start a task attempt, Vibe Kanban orchestrates several steps to prepare the workspace and execute your task:

1. **Workspace Preparation**: Vibe Kanban prepares the workspace for this task attempt
...
Vibe Kanban prepares the workspace according to the mode you selected. In the default recommended `git_worktree` mode, it creates an isolated Git worktree for the task attempt. In `in_place_git` and `in_place_directory` modes, it runs in the checkout or folder you selected instead of creating another managed copy.

<Info>
`git_worktree` mode gives you the strongest isolation. In in-place modes, the agent works directly in your selected directory.
</Info>
