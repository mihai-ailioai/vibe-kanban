...
<Note>
Each time a coding agent is executed it runs in the workspace you selected. In the default recommended `git_worktree` mode, that means a managed [git worktree](https://git-scm.com/docs/git-worktree) which is unlikely to contain your dependencies, configs, `.env`, and other local files.
</Note>
...
Comma-separated list of files to copy from the original project directory into the managed workspace copy when you use `git_worktree` mode. These files are copied after the worktree is created but before the setup script runs. Useful for environment-specific files like `.env`, configuration files, and local settings.
