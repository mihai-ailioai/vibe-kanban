...
<Info>
The MCP user experience stays repository-based. Internally, Vibe Kanban converts each `repositories` entry into the local API's canonical Git `sources` payload and sets `workspace_mode` to the default recommended `git_worktree` mode before it creates the workspace. MCP does not currently expose a separate workspace mode selector.
</Info>
...
4. **Start Workspaces**: Use `start_workspace` to programmatically begin work on issues with specific coding agents in the default recommended `git_worktree` mode
...
This creates a new workspace in the default recommended `git_worktree` mode, links it to the issue, creates a working branch from the target branch, and starts the coding agent in a managed Git worktree environment.
...
Each step uses the appropriate MCP tool (`list_organizations`, `list_projects`, `list_issues`, `create_issue`, `assign_issue`, `start_workspace`) to manage the complete workflow from planning to execution. In the final step, `start_workspace` still takes `repositories`, then creates the workspace in the default recommended `git_worktree` mode behind the scenes.
